import FiveStars from "../../assets/five-stars.png";

const carouselTestimonialsData = [
    {
        image: FiveStars,
        title: 'Nilo - *Specialized*',
        text: 'Quando começamos a pesquisar sobre uma empresa que oferesse esse tipo de serviço,  gostamos muito da proposta da Ecobraz. Fizemos essa parceria e hoje nossa instituição (AEB - CEDESP eTelecentro) se tornou um ponto de descarte e estamos juntos desde 2021.  Sempre muito solicitos, organizados e com um trabalho de extrema importância para o meio ambiente, temos certeza de que escolhemos a instituição certa.',
    },
    {
        image: FiveStars,
        title: 'Flavia - *PHX*',
        text: 'Já utilizo o serviço há muito tempo e o atendimento é simples e rápido, de acordo coma minha disponibilidade de dia e horário, sem custo nenhum!! É gratificante saber que podemos contar com uma empresa, preocupada com a conservação do nosso meio ambiente, destinando os descartes de forma correta. TODOS nós somos responsáveis pela conservação e preservação do nosso meio ambiente e cada um deve fazer a sua parte para que, JUNTOS, possamos atingir o objetivo de cuidar do nosso Planeta Terra.',
    },
    {
        image: FiveStars,
        title: 'andresa - *Specialized*',
        text: 'Quando começamos a pesquisar sobre uma empresa que oferesse esse tipo de serviço,  gostamos muito da proposta da Ecobraz. Fizemos essa parceria e hoje nossa instituição (AEB - CEDESP eTelecentro) se tornou um ponto de descarte e estamos juntos desde 2021.  Sempre muito solicitos, organizados e com um trabalho de extrema importância para o meio ambiente, temos certeza de que escolhemos a instituição certa.',
    },
    {
        image: FiveStars,
        title: 'paulo - *PHX*',
        text: 'Já utilizo o serviço há muito tempo e o atendimento é simples e rápido, de acordo coma minha disponibilidade de dia e horário, sem custo nenhum!!',
    },
    {
        image: FiveStars,
        title: 'renan - *Specialized*',
        text: 'Quando começamos a pesquisar sobre uma empresa que oferesse esse tipo de serviço,  gostamos muito da proposta da Ecobraz. Fizemos essa parceria e hoje nossa instituição (AEB - CEDESP eTelecentro) se tornou um ponto de descarte e estamos juntos desde 2021.  Sempre muito solicitos, organizados e com um trabalho de extrema importância para o meio ambiente, temos certeza de que escolhemos a instituição certa.',
    },
    {
        image: FiveStars,
        title: 'ana - *PHX*',
        text: 'Já utilizo o serviço há muito tempo e o atendimento é simples e rápido, de acordo coma minha disponibilidade de dia e horário, sem custo nenhum!! É gratificante saber que podemos contar com uma empresa, preocupada com a conservação do nosso meio ambiente, destinando os descartes de forma correta. TODOS nós somos responsáveis pela conservação e preservação do nosso meio ambiente e cada um deve fazer a sua parte para que, JUNTOS, possamos atingir o objetivo de cuidar do nosso Planeta Terra.',
    }
];

export default carouselTestimonialsData;